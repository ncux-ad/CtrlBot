...
def format_detection_info(text: str) -> str:
    ...
    if is_html_formatting(text):
        return "🔍 *Обнаружено HTML форматирование* - конвертировано в Markdown"
    elif any(marker in text for marker in ['*', '_', '`', '[', ']']):
        return "📝 *Обнаружено Markdown форматирование* - используется как есть"
    elif has_rich_formatting(text):
        return "✨ *Обнаружено форматирование* - применено базовое форматирование"
    else:
        return "📄 *Обычный текст* - без форматирования"
